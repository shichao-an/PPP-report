...
The \verb#time# function of Python's \verb#time# module will be used for timing programs (real time). The programs are non-trivial, so it is better to measure the wall-clock time to reflect the production environment. This is primary benchmark that comprises most of the evaluation result.

To further measure user and system times of the processes for analysis purpose, \verb#psutil# will be used as a general process status utility used in a programmatic manner \cite{psutil}. It is used to determine the types of programs by checking CPU usage and network connections. \verb#cProfile#, the standard profiler for Python, will be used to provide deterministic profiling for the programs to find bottlenecks.

As one of the major comparison criteria in the parallelization experiments, development efficiency needs manual evaluation from a different analysis perspective from that of performance. This means some subjectiveness may be related to conclude this criteria, such as programming effort to ensure interface compatibility and debugging difficulty (such as Cython's compile time check). However, code checking tools such as \textit{flake8} \cite{flake8} and \textit{cloc} \cite{cloc} will still be used to help measure development efficiency by providing substantial statistics like McCabe complexity (cyclomatic complexity) and lines of code.


\section{Experiments and Discussion}
The development efficiency and performances are major measures to make an evaluation overview of the parallelizing methods. 

Development efficiency is evaluated by comparing the parallelized source code with the sequential and measure the specific criteria: 1. The number of additional necessary interface used to perform parallelization and ensure correctness (thread-safe) 2. Extra effort to take to ensure interface compatibility with the original program; 3. Extra configuration and debugging steps; 4. Incremented complexity of the program.

Performance is evaluated: 1. Real time to run the parallelized program to see how much speedup or overhead is brought; 2. User/system time (larger than real time) and of the multi-threaded and multi-processing program to see how much parallelism is brought (particularly for OpenMP);


The expected results are case dependent, but can be generalized in terms of these measures

Development efficiency: \verb#threading# always have less coding effort and better compatibility than the other two, while Cython/OpenMP requires the most effort.

Performance: In terms of real time, \verb#multiprocessing# and Cython/OpenMP generally performs better than \verb#threading# in CPU intensive tasks, but \verb#threading# and Cython/OpenMP performs better in I/O-bound programs. In terms of parallelism, only Cython/OpenMP and \verb#multiprocessing# will bring native parallelism, while \verb#threading# cannot due to the restriction of GIL
...
\multicolumn{1}{C{4em}}{\bf thread/process=8} &
\multicolumn{1}{C{5em}}{\bf proc time} &
\multicolumn{1}{C{5em}}{\bf real time} &
\multicolumn{1}{C{5em}}{\bf user time} &
\multicolumn{1}{C{5em}}{\bf sys time} &
\multicolumn{1}{C{8em}}{\bf user time (work processes)} &
\multicolumn{1}{C{8em}}{\bf sys time (worker processes)} \\
\hline
cpu\_bound.mt & 21.1661 & 21.6565 & 19.1419 & 8.4130 & N/A & N/A  \\
cpu\_bound.mp & 7.1837 & 7.6333 & 0.2066 & 0.1090 & 23.8242 & 0.3836 \\
cpu\_bound.omp & 10.4938 & 10.9693 & 39.0557 & 0.1261 & N/A & N/A \\
io\_bound.mt & 2.9933 & 3.8690 & 0.8993 & 0.1509 & N/A & N/A \\
io\_bound.mp & 16.4212 & 17.2963 & 0.6450 & 0.1209 & 0.3836 & 0.1307 \\
io\_bound.omp & 2.9347 & 3.7645 & 0.8770 & 0.1439 & N/A & N/A \\
memory\_bound.mt & 19.7836 & 20.3949 & 19.1920 & 2.6024 & N/A & N/A \\
memory\_bound.mp & 8.2920 & 8.9134 & 0.3793 & 1062 & 31.9653 & 0.1055 \\
memory\_bound.omp & 0.7290 & 1.3544 & 3.2256 & 0.1005 & N/A & N/A \\
overhead.mt & 0.1348 & 0.5210 & 0.2539 & 0.1660 & N/A & N/A \\
overhead.mp & 4.2435 & 4.7062 & 0.2252 & 0.1375 & 3.6614 & 2.5615 \\
overhead.omp & 0.0983 & 0.5214 & 0.2232 & 0.1500 & N/A & N/A \\
\end{tabular}
