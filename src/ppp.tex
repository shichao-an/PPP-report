...
\setlength{\itemsep}{0em}
  ...
  \item[\it \it mp (manager):] \raggedright parallelized version using \verb#multiprocessing# approach with data types supported by \verb#Manager#
...
Statistical: 1. $L$: Total incremental lines of code; 2. $M$ mccabe complexity: $\sum\limits_{i=1}^n (c_i - 2) $ 3. $I$ Additional interfaces used 4. $C$ Lines of code in converting data structure for input and output 5. $E$ Extra configuration 
...
Debugging difficulty in Cython is that every time the code is changed, it must be re-complied.

General aspects
Create a table of descriptive criteria for the three approaches: 
